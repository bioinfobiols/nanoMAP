...
def test_reference_configs_are_portable_and_explicit(
    ...
    assert payload["input"][input_key].startswith("../data/d2/")
    ...
    assert config.input.trusted_pickle is False
    assert config.input.cell_segmentation.suffix == ".npz"
    assert all(path.is_file() for path in (
        getattr(config.input, input_key), config.input.laser_image, config.input.cell_segmentation
    ))
...
@pytest.mark.parametrize("dataset", ["d2"])
