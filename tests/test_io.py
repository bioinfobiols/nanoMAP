...
def test_read_segmentation_supports_compressed_integer_labels(tmp_path: Path):
    labels = np.array([[0, 2, 2], [7, 0, 7]], dtype=np.int64)
...
@pytest.mark.parametrize("payload", [
    {"wrong_key": np.zeros((2, 2), dtype=int)},
    {"labels": np.zeros((2, 2), dtype=int), "extra": np.zeros(1)},
    {"labels": np.zeros((2, 2), dtype=float)},
    {"labels": np.zeros((1, 2, 2), dtype=int)},
    {"labels": np.array([[object()]], dtype=object)},
])
def test_read_segmentation_rejects_invalid_compressed_labels(tmp_path: Path, payload):
    path = tmp_path / "labels.npz"
